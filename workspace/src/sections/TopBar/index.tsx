...
const content = advertorial as AdvertorialContent;
const media = advertorialMedia as AdvertorialMedia;
...
function getAvatarSrc(id: string) {
  return media.comments.avatars[id] ?? "";
}
...
const CommentEntry = ({ c }: { c: Comment }) => {
  ...
  return (
    <li className="items-start flex mt-2.5 reveal-fast">
      <img src={getAvatarSrc(c.id)} alt={c.name} className="float-left h-12 w-[50px] mr-2 mt-[5px] rounded-none object-cover" />
          ...
          <ul className="pl-0 mt-1 border-l-2 border-gray-200 ml-1">
            {c.replies.map((reply, index) => (
              <li key={`${reply.id}-${index}`} className="items-start flex mt-2 pl-2">
                <img src={getAvatarSrc(reply.id)} alt={reply.name} className="float-left h-9 w-10 mr-2 mt-[5px] rounded-none object-cover" />
