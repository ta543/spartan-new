...
You generate production-quality SVG icons by vectorizing design mockup screenshots using the vectosolve MCP service, then post-processing the result into clean 24×24 white-on-transparent SVGs.
...
1. Generate a random 8-character hex string for `HASH`: `openssl rand -hex 4`
2. **IMMEDIATELY** save the clipboard image:
   ```bash
   HASH=$(openssl rand -hex 4) && if [[ "$OSTYPE" == darwin* ]]; then \
     osascript -e 'set theFile to (path to temporary items as string) & "clipboard.png"' \
               -e 'do shell script "screencapture -c /tmp/clip_tmp.png" & " 2>/dev/null || true"' && \
     pngpaste ".ai/icon_${HASH}.png" 2>/dev/null || \
     osascript -e 'tell app "System Events" to keystroke "v" using command down' ; \
   else \
     powershell -Command "Add-Type -AssemblyName System.Windows.Forms; \
       \$img=[System.Windows.Forms.Clipboard]::GetImage(); \
       if(\$img){\$img.Save('.ai/icon_${HASH}.png')}" ; \
   fi
   ```
   If the command fails (no image on clipboard): tell the user "Please copy the icon area first, then retry" (macOS: Cmd+Ctrl+Shift+4; Windows: Win+Shift+S) and **STOP**.

3. Read back `.ai/icon_HASH.png` with the Read tool and compare visually with the pasted image.
   - If they look **completely different**: delete the file and stop with: "Clipboard image doesn't match. Please re-copy and retry."
   - If they match: proceed.

If NO image is attached to the message, skip this step.
...
1. Parse `$ARGUMENTS` to determine:
   ...
   - **Icon type**: default is `ui` (24×24 UI icon). User may specify otherwise.
   - **Target path**: where the final SVG should be placed. Ask the user if not specified.

2. Choose an icon file name:
   - Lowercase letters and underscores only — **NO hyphens**
   - Must NOT conflict with existing SVG files at the target path
...
4. Write `.ai/icon_{name}/context.md` with:
   ...
   ## Icon: {icon_name}
   Type: {ui/other}
   Target: {user-specified path or TBD}
...
Set `RENDER_AVAILABLE = false` — SVG rendering will be done visually via the Read tool on the final SVG file. No external renderer is required.
...
Read the SVG file `.ai/icon_{name}/{LETTER}.svg` with the Read tool to visually verify the paths look correct. Assess: recognizable shape, reasonable scale, clean paths, white-on-transparent colors.

## Phase 2: Review

Assess the SVG visually:

1. **Recognizable?** Clearly identifiable as the intended symbol.
2. **Scale reasonable?** Fills the space with ~2-3px padding.
3. **Clean paths?** No broken paths, artifacts, or unwanted elements.
...
If there are fixable issues (stray element, missed color, wrong viewBox) → fix the SVG directly and re-check.

If the result is poor (vectosolve couldn't handle the input) → report to the user and suggest:
- A cleaner/larger crop of the icon area
- A different screenshot
- Following up: `/icon {icon_name} <description of what to change>`

## Phase 3: Output

1. Read the `Target:` line from `.ai/icon_{name}/context.md` to get the output path. If it's "TBD", ask the user where to place the SVG.

2. Copy the final SVG to that target path.

3. Update `.ai/icon_{name}/context.md` — append:
   ```
   ## Latest Output
   Letter: {LETTER}
