...
This pair can then be referenced when running `/task` or directly by an agent.
...
```
/planner schema-upgrade
```
Creates `.ai/schema-upgrade/` with prompt and tasks for migrating database schema changes.
